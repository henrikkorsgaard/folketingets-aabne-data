...
sqlite_conn = sqlite3.connect("data/oda.test.sqlite.db")
...
# Create the database
with open("sql/oda.test.sqlite.sql", "r") as sqlfile:
    sqlite_cursor.executescript(sqlfile.read())

for table in tables:
    print("Ingesting 15000 rows from MSSQL oda." + table + " into SQLite oda."+ table)
    mssql = "SELECT TOP(15000) * FROM " + table + " ORDER BY '" + pkeys[table] + "' ASC" # All the pkeys are id, but you never know
